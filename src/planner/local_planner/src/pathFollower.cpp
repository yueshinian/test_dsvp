...
double yawRateGain = 7.5; // 非低速情况下得到角速度的参数
double stopYawRateGain = 7.5; // 低速情况下得到角速度的参数
...
void odomHandler(const nav_msgs::Odometry::ConstPtr& odomIn)
{
  ...
  // 停止运动 useInclToStop = false
  if ((fabs(roll) > inclThre * PI / 180.0 || fabs(pitch) > inclThre * PI / 180.0) && useInclToStop) {
    stopInitTime = odomIn->header.stamp.toSec();
  }
  // 减速运动 useInclRateToSlow = false
...
int main(int argc, char** argv)
{
  ...
  nhPrivate.getParam("cmd_vel", msCmdVel);
  ...
  cmd_vel.header.frame_id = path_head;
  
  // 无时间戳的速度
  ros::Publisher pubSpeed_dpc = nh.advertise<geometry_msgs::Twist> (msCmdVel, 5);
  geometry_msgs::Twist cmd_vel_dpc;
  ...
  bool status = ros::ok();
  int count = 0; // fzq
  double angular_count = 0.0; // fzq
  double linear_count = 0.0; // fzq
  while (status) {
    ros::spinOnce();
    ...
    if (pathInit) {
      ...
      disY = path.poses[pathPointID].pose.position.y - vehicleYRel;
      dis = sqrt(disX * disX + disY * disY);
      float pathDir = atan2(disY, disX); // path的方向，车体坐标系下
      ...
      // 生成角速度
      // maxAccel = 1.0 stopYawRateGain = 7.5 yawRateGain = 7.5 maxYawRate = 45.0
      ...
      // stopDisThre = 0.2, noRotAtGoal = true dirDiffThre = 0.1 slowRate = 0.25
      if (pathSize <= 1) {
        joySpeed2 = 0;
      } else if (endDis / slowDwnDisThre < joySpeed) {
        joySpeed2 *= endDis / slowDwnDisThre; // slowDwnDisThre = 1.0
      }

      float joySpeed3 = joySpeed2;
      // slowInitTime = odomIn->header.stamp.toSec(), slowTime = 2.0
      ...
      // stopInitTime = odomIn->header.stamp.toSec(), stopTime = 5.0
      ...
      if (pubSkipCount < 0) {
        ...
        if (fabs(vehicleSpeed) <= maxAccel / 100.0) {
          ...
          cmd_vel_dpc.linear.x = 0;
        }
        else {
          //cmd_vel.twist.linear.x = vehicleSpeed;
          //cmd_vel_dpc.linear.x = vehicleSpeed;
          linear_count += vehicleSpeed;  // fzq
        }
        //cmd_vel.twist.angular.z = vehicleYawRate;
        //cmd_vel_dpc.angular.z = vehicleYawRate;
        angular_count += vehicleYawRate; // fzq
        count++;
        //cout << count << endl;
        if (count == 5) {
            cmd_vel.twist.linear.x = linear_count/5.0;  // fzq
            cmd_vel.twist.angular.z = angular_count/5.0;

            if (fabs(cmd_vel.twist.angular.z) < 0.15) 
                cmd_vel.twist.angular.z = 0.0;
            cmd_vel_dpc.linear.x = cmd_vel.twist.linear.x; // fzq
            cmd_vel_dpc.angular.z = cmd_vel.twist.angular.z; // fzq
            pubSpeed.publish(cmd_vel);
            pubSpeed_dpc.publish(cmd_vel_dpc);
            //cout << "get !" << endl;
            linear_count = 0.0;
            angular_count = 0.0;
            count = 0;
        ...
        //pubSpeed.publish(cmd_vel);
        //pubSpeed_dpc.publish(cmd_vel_dpc);
        pubSkipCount = pubSkipNum; // pubSkipNum = 1.0
      }
    }
