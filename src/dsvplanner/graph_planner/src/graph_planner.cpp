...
bool GraphPlanner::goToPoint(geometry_msgs::Point point) {
  ...
  //lym
  typedef std::pair<float, int> iPair;
  std::priority_queue<iPair, std::vector<iPair>, std::greater<iPair>> robot_q,goal_q;
  double cur_best_idx = -1, goal_best_idx=-1;
  double cur_best_distance = INFINITY, goal_best_distance=INFINITY;
  for (int v_idx = 0; v_idx < planned_graph_.vertices.size(); ++v_idx) {
    double distance_cur =
        misc_utils_ns::PointXYZDist(robot_pos_, planned_graph_.vertices[v_idx].location);
      double distance_goal =
        misc_utils_ns::PointXYZDist(point, planned_graph_.vertices[v_idx].location);
      robot_q.push(std::make_pair(distance_cur,v_idx));
      goal_q.push(std::make_pair(distance_goal,v_idx));
  }
  int current_vertex_idx=0,goal_vertex_idx=0;
  while(!robot_q.empty()){
    auto [dis,id]=robot_q.top();
    if(!collisionCheckByTerrain(robot_pos_,id)){
      current_vertex_idx=id;
      break;
    }else{
      robot_q.pop();
    }
  ...
  while(!goal_q.empty()){
    auto [dis,id]=goal_q.top();
    if(!collisionCheckByTerrain(point,id)){
      goal_vertex_idx=id;
      break;
    }else{
      goal_q.pop();
  ...
  */
  ...
  //auto [dis_cur,id_cur]=robot_q.top();
  //auto [dis_goal,id_goal]=goal_q.top();
  return goToVertex(current_vertex_idx, goal_vertex_idx);
}
...
bool GraphPlanner::initialize() {
  ...
  home_point.point.z=0.0;
  home_point.header.frame_id = world_frame_id_;
  ...
  //lym
  subHomePoint = nh_.subscribe("/homePoint",1, &GraphPlanner::homePointCallback,this);
...
void GraphPlanner::homePointCallback(const geometry_msgs::PointStamped::ConstPtr &point_msg){
  home_point.point.x=point_msg->point.x;
  home_point.point.y=point_msg->point.y;
  home_point.point.z=point_msg->point.z;
}
...
void GraphPlanner::executeGoToOrigin() {
  ...
  origin.z = 0;
  */
  origin.x = home_point.point.x;
  origin.y = home_point.point.y;
  origin.z = home_point.point.z;
...
void GraphPlanner::executeCommand() {
  ...
  if (graph_planner_command_.command ==
    ...
    returnHomeStatu=true;//lym，停止更新RRT拓扑网络
    executeGoToOrigin();
  } else if (graph_planner_command_.command ==
             graph_planner::GraphPlannerCommand::COMMAND_GO_TO_LOCATION) {
               returnHomeStatu=false;
