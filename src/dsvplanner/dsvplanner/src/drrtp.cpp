...
bool dsvplanner_ns::drrtPlanner::setParams() {
  ...
  //lym
  nh_private_.getParam("/changeStep", params_.changeStep);
  nh_private_.getParam("/gainMode", params_.gainMode);
  nh_private_.getParam("/globalChoose", params_.globalChoose);
  return true;
}
...
bool dsvplanner_ns::drrtPlanner::init() {
  ...
  params_.changeStepPub =
      nh_.advertise<sensor_msgs::PointCloud2>("changeStepPub", 1000);   //lym
