...
void dsvplanner_ns::Drrt::init() {
  kMinextensionRange = params_.kMinextensionRange;//lym
  kExtensionRange = params_.kExtensionRange;//lym
  kdTree_ = kd_create(3);
  iterationCount_ = 0;
...
void dsvplanner_ns::Drrt::plannerIterate() {
  ...
  //lym
  bool isChangeStep = params_.changeStep;
  if(isChangeStep){
  StateVec lastState = newState;
  dsvplanner_ns::Node *lastParent = newParent;
  double lastDistance = newParent->distance_ + direction.norm();
  int extensionIt = 0;
  bool extensionItFlag =false;
  for(int i = 2;  i< int(15.0/params_.kExtensionRange); i++) {
    StateVec endPoint = origin +i*direction;
    newState[0] = endPoint[0];
    newState[1] = endPoint[1];
    ...
    if (!inPlanningBoundary(newState)) {
      ...
      if (volumetric_mapping::OctomapManager::CellStatus::kFree ==
              manager_->getLineStatusBoundingBox(origin, newState,
                                                params_.boundingBox) &&
          (!grid_->collisionCheckByTerrainWithVector(origin,newState))) { // connection is free. Create new node and insert into tree
          lastState = newState;
          lastParent = newParent;
          lastDistance = newParent->distance_ + direction.norm();
          extensionItFlag = true;
          extensionIt=i;
          continue;
      }
    }
  ...
  if(extensionIt>1 && extensionItFlag){
      dsvplanner_ns::Node *newNode = new dsvplanner_ns::Node;
      newNode->state_ = lastState;
      newNode->parent_ = lastParent;
      newNode->distance_ = lastDistance;
      newParent->children_.push_back(newNode);
      newNode->gain_ = gain(newNode->state_);

      kd_insert3(kdTree_, lastState.x(), lastState.y(), lastState.z(), newNode);

      geometry_msgs::Pose p1;
      p1.position.x = lastState.x();
      p1.position.y = lastState.y();
      p1.position.z = lastState.z();
      p1.orientation.y = newNode->gain_;
      dual_state_graph_->addNewLocalVertexWithoutDuplicates(
          p1, dual_state_graph_->local_graph_);
      dual_state_graph_->execute();
...
void dsvplanner_ns::Drrt::plannerInit() {
  ...
  params_.kMinextensionRange = kMinextensionRange;//lym->change step
  params_.kExtensionRange= kExtensionRange;//lym
  if(params_.changeStep){
    changeStep(root_,  params_.kMaxXLocalBound- params_.kMinXLocalBound);  //lym
  }
...
double dsvplanner_ns::Drrt::getSurroundGain(StateVec state){
  const double disc = manager_->getResolution();
  std::vector<double> dir={-disc,0,disc,0,-disc};
...
if(count_free && count_occu){//有free和occupied和unknown
  sur_gain=std::max(count_free-count_occu,0);
}
else if(count_free){//只有free和unknown
  sur_gain=count_free;
}
else if(count_occu){//只有occupied
  sur_gain=-0;
}
else{//只有unknown
  sur_gain=0;
...
double dsvplanner_ns::Drrt::gain(StateVec state) {
  ...
  int mode = params_.gainMode;//lym
  int countUnknown=0,countOccu=0,countFree=0;//lym
  double gain = 0.0;
  const double disc = manager_->getResolution();
  StateVec origin(state[0], state[1], state[2]);
  StateVec vec;
  double rangeSq = pow(params_.kGainRange, 3.0);
      ...
      for (vec[2] = std::max(state[2] - params_.kGainRangeZMinus, minZ_);
        ...
        if (node == volumetric_mapping::OctomapManager::CellStatus::kUnknown) {
          if (volumetric_mapping::OctomapManager::CellStatus::kOccupied !=
            ...
            gain += params_.kGainUnknown + mode*getSurroundGain(vec); //lym
            ++countUnknown;
          }
        } else if (node ==
  ...
  if(mode==2){
    gain=std::max(countUnknown+countFree-countOccu,countUnknown);
  }
  // Scale with volume
...
//lym
void dsvplanner_ns::Drrt::changeStep(
    const StateVec &center, double bounding_box_size) {
  pcl::PointCloud<pcl::PointXYZ>::Ptr sample_points_ =
      pcl::PointCloud<pcl::PointXYZ>::Ptr(new pcl::PointCloud<pcl::PointXYZ>());

  // Determine correct center of voxel.
  double kFrontierResolution=0.3,  kVehicleHeight=0.9;
  ...
  double dis1=0,dis2=0,dis3=0,dis4=0,dis5=0,dis6=0,dis7=0,dis8=0;
  double disx=0,disy=0,usedis=0,disen=0,dises=0;
  double checkThrehold=3.6;
  for(int i=1; i< bounding_box_size/2; i++){
      double x1 = center_corrected[0] + i*kFrontierResolution;
      double y1 = center_corrected[1] ;
      double z1 = getZvalue(x1,y1);
      octomap::point3d point1 = octomap::point3d(x1, y1, z1);
      octomap::OcTreeKey key1 = manager_->octree_->coordToKey(point1);
      octomap::OcTreeNode *node1 = manager_->octree_->search(key1);
      if  (node1 != NULL && (!manager_->octree_->isNodeOccupied(node1)) &&flag1 && dis1<checkThrehold) {
        sample_points_->push_back(pcl::PointXYZ(point1.x(), point1.y(), point1.z()));
        dis1+=kFrontierResolution;
      }else{
        flag1=false;
      }

      double x2 = center_corrected[0] - i*kFrontierResolution;
      double y2 = center_corrected[1] ;
      double z2 = getZvalue(x2,y2);
      octomap::point3d point2 = octomap::point3d(x2, y2, z2);
      octomap::OcTreeKey key2 = manager_->octree_->coordToKey(point2);
      octomap::OcTreeNode *node2 = manager_->octree_->search(key2);
      if  (node2 != NULL && (!manager_->octree_->isNodeOccupied(node2)) &&flag2 &&dis2<checkThrehold) {
        sample_points_->push_back(pcl::PointXYZ(point2.x(), point2.y(), point2.z()));
        dis2+=kFrontierResolution;
      }else{
        flag2=false;
      }
      disx=std::max(dis1,dis2);

      double x3 = center_corrected[0] ;
      double y3 = center_corrected[1] + i*kFrontierResolution;
      double z3 = getZvalue(x3,y3);
      octomap::point3d point3 = octomap::point3d(x3, y3, z3);
      octomap::OcTreeKey key3 = manager_->octree_->coordToKey(point3);
      octomap::OcTreeNode *node3 = manager_->octree_->search(key3);
      if  (node3 != NULL && (!manager_->octree_->isNodeOccupied(node3)) &&flag3 &&dis3<checkThrehold) {
        sample_points_->push_back(pcl::PointXYZ(point3.x(), point3.y(), point3.z()));
        dis3+=kFrontierResolution;
      }else{
        flag3=false;
      }

      double x4 = center_corrected[0] ;
      double y4 = center_corrected[1] - i*kFrontierResolution;
      double z4 = getZvalue(x4,y4);
      octomap::point3d point4 = octomap::point3d(x4, y4, z4);
      octomap::OcTreeKey key4 = manager_->octree_->coordToKey(point4);
      octomap::OcTreeNode *node4 = manager_->octree_->search(key4);
      if  (node4 != NULL && (!manager_->octree_->isNodeOccupied(node4)) &&flag4 &&dis4<checkThrehold) {
        sample_points_->push_back(pcl::PointXYZ(point4.x(), point4.y(), point4.z()));
        dis4+=kFrontierResolution;
      }else{
        flag4=false;
      }

      double x5 = center_corrected[0] + int(0.707*i)*kFrontierResolution;
      double y5 = center_corrected[1] + int(0.707*i)*kFrontierResolution;
      double z5 = getZvalue(x5,y5);
      octomap::point3d point5 = octomap::point3d(x5, y5, z5);
      octomap::OcTreeKey key5 = manager_->octree_->coordToKey(point5);
      octomap::OcTreeNode *node5 = manager_->octree_->search(key5);
      if  (node5 != NULL && (!manager_->octree_->isNodeOccupied(node5)) &&flag5 &&dis5<checkThrehold) {
        sample_points_->push_back(pcl::PointXYZ(point5.x(), point5.y(), point5.z()));
        dis5+=kFrontierResolution;
      }else{
        flag5=false;
      }

      double x6 = center_corrected[0] - int(0.707*i)*kFrontierResolution;
      double y6 = center_corrected[1] + int(0.707*i)*kFrontierResolution;
      double z6 = getZvalue(x6,y6);
      octomap::point3d point6 = octomap::point3d(x6, y6, z6);
      octomap::OcTreeKey key6 = manager_->octree_->coordToKey(point6);
      octomap::OcTreeNode *node6 = manager_->octree_->search(key6);
      if  (node6 != NULL && (!manager_->octree_->isNodeOccupied(node6)) &&flag6 && dis6<checkThrehold) {
        sample_points_->push_back(pcl::PointXYZ(point6.x(), point6.y(), point6.z()));
        dis6+=kFrontierResolution;
      }else{
        flag6=false;
      }

      double x7 = center_corrected[0] - int(0.707*i)*kFrontierResolution;
      double y7 = center_corrected[1] - int(0.707*i)*kFrontierResolution;
      double z7 = getZvalue(x7,y7);
      octomap::point3d point7 = octomap::point3d(x7, y7, z7);
      octomap::OcTreeKey key7 = manager_->octree_->coordToKey(point7);
      octomap::OcTreeNode *node7 = manager_->octree_->search(key7);
      if  (node7 != NULL && (!manager_->octree_->isNodeOccupied(node7)) &&flag7 &&dis7<checkThrehold) {
        sample_points_->push_back(pcl::PointXYZ(point7.x(), point7.y(), point7.z()));
        dis7+=kFrontierResolution;
      }else{
        flag7=false;
      }

      double x8 = center_corrected[0] + int(0.707*i)*kFrontierResolution;
      double y8 = center_corrected[1]  - int(0.707*i)*kFrontierResolution;
      double z8 = getZvalue(x8,y8);
      octomap::point3d point8 = octomap::point3d(x8, y8, z8);
      octomap::OcTreeKey key8 = manager_->octree_->coordToKey(point8);
      octomap::OcTreeNode *node8 = manager_->octree_->search(key8);
      if  (node8 != NULL && (!manager_->octree_->isNodeOccupied(node8)) &&flag8 && dis8<checkThrehold) {
        sample_points_->push_back(pcl::PointXYZ(point8.x(), point8.y(), point8.z()));
        dis8+=kFrontierResolution;
      }else{
        flag8=false;
      }
  ...
  double minExtention=1000;
  if(std::max(dis1,dis3)<params_.kExtensionRange){
    minExtention = std::min(minExtention,std::max(dis1,dis3));
  }
  if(std::max(dis2,dis4)<params_.kExtensionRange){
    minExtention = std::min(minExtention,std::max(dis1,dis3));
  }
  if(std::max(dis5,dis7)<params_.kExtensionRange){
    minExtention = std::min(minExtention, std::max(dis1,dis3));
  }
  if(std::max(dis6,dis8)<params_.kExtensionRange){
    minExtention = std::min(minExtention, std::max(dis1,dis3));
  }
  double maxExtension1=std::max(dis1, std::max(dis2, std::max(dis3,dis4))), maxExtension2= std::max(dis2, std::max(dis2, std::max(dis3,dis4)));
  if(maxExtension1 < kExtensionRange || maxExtension2 < kExtensionRange){
    params_.kExtensionRange = std::min(maxExtension1,maxExtension2);
  }
  if(params_.kMinextensionRange > minExtention){
    params_.kMinextensionRange = minExtention;
  }
  params_.kMinextensionRange = ( (double)( (int)( (params_.kMinextensionRange+0.05)*10 ) ) )/10; //取一位小数
  params_.kExtensionRange = ( (double)( (int)( (params_.kExtensionRange+0.05)*10 ) ) )/10; //取一位小数
  if(params_.kMinextensionRange<0.2){
    params_.kMinextensionRange=0.2;
  }
  if(params_.kExtensionRange<0.4){
    params_.kExtensionRange=0.4;
  }
  std::cout<<"kMinextensionRange = "<<params_.kMinextensionRange<<std::endl;
  std::cout<<"kExtensionRange = "<<params_.kExtensionRange<<std::endl;
  sensor_msgs::PointCloud2 sample_pcl;
  pcl::toROSMsg(*sample_points_, sample_pcl);
  sample_pcl.header.frame_id = "map";
  sample_pcl.header.stamp = ros::Time::now();
  params_.changeStepPub.publish(sample_pcl);
}
